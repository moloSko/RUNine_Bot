...
/**
 * @author    Moloko
 * @link      https://github.com/Nizarii/arma-rcon-php-class Github repository of NAS
 * @version   2.2
 */
...
{
    /**
     * @var array Options for NAS stored in an array
     */
    private $options = [
        'timeoutSec'    => 1,
        'autosaveBans'  => false,
     ...
     * @param array $options        Options array of NAS
     *
    ...
    /**
     * Checks if NAS's option array contains any deprecated options
     */
    ...
    /**
     * Returns the socket used by NAS, might be null if connection is closed
     *
     * @return resource
     */
    ...
    /**
     * Отправляет пользовательскую команду на сервер
     *
     * @param string $command Команда, которая будет отправлена на сервер
     *
     * @throws \Exception if wrong parameter types were passed to the function
     *
    ...
    public function kickPlayer($player, $reason = 'Admin Kick')
    {
        if (!is_int($player) && !is_string($player)) {
            throw new \Exception(
                sprintf('Ожидаемый параметр 1 должен быть строковым или целым числом, получено %s', gettype($player))
            );
        }
        if (!is_string($reason)) {
            throw new \Exception(
                sprintf('Ожидаемый параметр 2 должен быть строковым, получено %s', gettype($reason))
            );
        }
    ...
    /**
     * Отправляет глобальное сообщение всем игрокам
     *
     * @param string $message Сообщение, которое будет показано всем игрокам
    ...
    /**
     * Выводит информацию о сервере(мониторинг)
     *
     * @param integer Будет выведено отдельно вам в консоль

     * @throws \Exception если в функцию были переданы неправильные типы параметров
     *
     * @return NAS
     */
    public function getMonitor($init = 1)
    {
        if (!is_int($init)) {
            throw new \Exception(
                sprintf('Ожидаемый параметр 1 должен быть числовым, получен %s', gettype($init))
            );
        }

        $this->send("monitor $init");
        return $this;
    }

    /**
     * Отправляет сообщение определенному игроку
     *
     * @param integer $player Игрок, которому будет отправлено сообщение
     * @param string $message Сообщение для игрока
    ...
    public function sayPlayer($player, $message)
    {
        if (!is_int($player) || !is_string($message)) {
            throw new \Exception('Неправильный тип(ы) параметра(ов)!');
        }
    ...
    /**
     * Загружает "scripts.txt " файл без необходимости перезагрузки сервера
     *
     * @return NAS
     */
    ...
    /**
     * Изменяет максимальное значение Ping. Если у игрока более высокий пинг, он будет удален с сервера
     *
    ...
    /**
     * ((Повторно) загрузите список запрещенных из bans.txt
     *
     * @return NAS
     */
    ...
    /**
     * Получает список всех игроков, находящихся в данный момент на сервере
     *
    ...
    /**
     * Получает список всех игроков, находящихся в данный момент на сервере, в виде массива
     *
    ...
    /**
     * Запретить игроку удаляться с сервера. Если время не указано или равно 0, запрет будет постоянным;.
     * Если причина не указана, игрок будет удален с сообщением "Забанен(DISCORD)".
     *
     ...
     * @return NAS
     */
    public function banPlayer($player, $reason = 'Забанен(DISCORD)', $time = 0)
    {
        if (!is_string($player) && !is_int($player)) {
            throw new \Exception(
                sprintf('Ожидаемый параметр 1 должен быть целым числом или строкой, получено %s', gettype($player))
            );
        }
    ...
    /**
     * То же, что и "banPlayer", но позволяет забанить игрока, которого в данный момент нет на сервере
     *
     * @param integer $player Player who will be banned
     * @param string $reason  Reason why the player is banned
     * @param integer $time   How long the player is banned in minutes (0 = permanent)
    ...
    /**
     * Получает текущую версию сервера BE
     *
    ...
    */
    ...
    /**
    * Читает, что это за упаковка. Этот метод также является вспомогательным для sequence.
    *
    * @author steffalon (https://github.com/steffalon)
    * @link https://github.com/schaeferfelix/arma-rcon-class-php/issues/30 issue part 1
    * @link https://github.com/schaeferfelix/arma-rcon-class-php/issues/31 issue part 2
    ...
    */
    ...
    /**
    * Считайте формат пакета и возвращайте преобразованный в пригодные для использования байты. Массив начинается с 0x[FF].
    *
    * @author steffalon (https://github.com/steffalon)
    *
    ...
    */
    ...
    /**
    * Подтвердите данные и добавьте +1 к последовательности.
    *
    ...
    */
    private function acknowledge($int)
    {
        ...
        if ($this->writeToSocket($statusmsg) === false) {
            throw new \Exception('Не удалось отправить команду!');
        }
        return ++$int; // Последовательность +1
    }

    /**
    * Поддерживайте поток живым. Отправьте пакет на сервер BE. Используйте эту функцию до истечения 45 секунд.
    *
    * @throws \Exception если не удалось отправить команду
    *
    * @return boolean
    */
    ...
    {
        if ($this->options['debug']) {
            echo '--Поддерживайте связь живой--'.PHP_EOL;
        }
        ...
        if ($this->writeToSocket($keepalive) === false) {
            throw new \Exception('Не удалось отправить команду!');
            return false; // Неудачный
        }
        return true; // Завершенный
    }

    /**
     * Преобразует текстовый список "array" в массив
     *
     * @param $str array
     *
     * @return array
    ...
    private function formatList($str)
    {
        ...
        array_shift($str);
        // Создать возвращаемый массив
        $result = array();

        // Цикл true для основных массивов, каждый из которых содержит значение
        foreach($str as $key => $value) {
            // Объединяет каждое основное значение в новый массив
